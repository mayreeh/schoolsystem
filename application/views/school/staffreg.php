			...
			<div class="main-container inner">
				...
				<div class="main-content">
						...
						<!-- start: TOOLBAR -->
							<div class="toolbar row">
								<div class="col-sm-6 hidden-xs">
									...
									<h3>  Staff Registration  <small></small></h3>
									</div>
								</div>
					...
					<form method="post" action="<?php echo base_url();?>index.php/school/staffreg" role="form" class="smart-wizard form-horizontal" id="form" enctype="multipart/form-data">
			             	<input type="hidden" name="staff_id" value="<?php if (!empty($staff_id)) { echo $staff_id; } ?> ">
						<?php  if (!empty($staff_id)) { $getStaff = $this->load->main_model->getStaffsById($staff_id);  } ?>
									...
									<div id="step-1">
										<h2 class="StepTitle">Step 1 Basic Information</h2>
										       <div class="form-group"><label class="col-sm-3 control-label">First Names <span class="symbol required"></span></label>
														<div class="col-sm-7"><input type="text" class="form-control" id="firstname" name="firstname" value = "<?php   if (!empty($staff_id)) { echo $getStaff[0]['firstname'];} ?>" placeholder="First name" required></div>
												</div>
												<div class="form-group"><label class="col-sm-3 control-label">Last Names <span class="symbol required"></span></label>
														<div class="col-sm-7"><input type="text" class="form-control" id="lastname" name="lastname" value = "<?php  if (!empty($staff_id)) { echo $getStaff[0]['lastname']; } ?>"   placeholder="Last name" required></div>
												</div>
												<div class="form-group"><label class="col-sm-3 control-label">Other Names <span class="symbol required"></span></label>
														<div class="col-sm-7"><input type="text" class="form-control" id="othername" name="othername"  value = "<?php  if (!empty($staff_id)) { echo $getStaff[0]['othername']; } ?>"  placeholder="Other name" required></div>
												</div>
													...
													<div class="col-sm-7">
													 ...
													 <option value="">-Select Role - </option>
								                        <?php foreach ($this->load->main_model->getSchoolRoles() as $roles){?>
								                            <option value="<?php echo $roles['role'] ; ?>" <?php  if (!empty($staff_id)) {  if ($getStaff[0]['role'] == $roles['role']) {?>  selected="selected" <?php } } ?> ><?php echo  $roles['role']; ?></option>
								                    <?php } ?>
								                   </select>
								                   </div>		
								             	</div>
									...
									<div id="step-2">
										<h2 class="StepTitle">Step 3 Upload Photo if available.</h2>
										   <div class="form-group" style="margin-left: 3%;">
											 ...
											 <?php if (!empty($getStaff[0]['file'])){ $prof = $getStaff[0]['file']; ?>
									...
									<div id="step-3">
										<h2 class="StepTitle">Step 3 Other Information</h2>
											    <div class="form-group"><label class="col-sm-3 control-label">ID Number <span class="symbol required"></span></label>
														<div class="col-sm-7"><input type="number" class="form-control" id="idnumber" name="idnumber" placeholder="ID Number"  value = "<?php  if (!empty($staff_id)) { echo $getStaff[0]['idnumber']; }?>"   required></div>
												</div>
												<div class="form-group"><label class="col-sm-3 control-label">Phone Number <span class="symbol required"></span></label>
														<div class="col-sm-7"><input type="number" class="form-control" id="phonenumber" name="phonenumber" placeholder="Phone Number" value = "<?php  if (!empty($staff_id)) { echo $getStaff[0]['phonenumber'];} ?>"   required></div>
												</div>
												<div class="form-group"><label class="col-sm-3 control-label">Email <span class="symbol required"></span></label>
														<div class="col-sm-7"><input type="email" class="form-control" id="email" name="email" placeholder="Email"  value = "<?php   if (!empty($staff_id)) { echo $getStaff[0]['email']; } ?>"   required></div>
												</div>
												<div class="form-group"><label class="col-sm-3 control-label">Gender <span class="symbol required"></span></label>
														<div class="col-sm-7">
															<label class="radio-inline"><input type="radio" class="grey" value="Female" name="gender" id="gender_female"   <?php  if (!empty($staff_id)) { if ($getStaff[0]['gender'] == "Female") {?> checked="checked" <?php } } ?>>Female</label>
															<label class="radio-inline"><input type="radio" class="grey" value="Male" name="gender"  id="gender_male"  <?php   if (!empty($staff_id)) { if ($getStaff[0]['gender'] == "Male") {?> checked="checked" <?php } } ?>>Male</label>
														</div>
												</div>
									...
									<div id="step-4">
										<h2 class="StepTitle">Step 4 Login Details</h2>
												<div class="form-group"><label class="col-sm-3 control-label">Username<span class="symbol required"></span></label>
														<div class="col-sm-7"><input type="text" class="form-control" id="username" name="username"  value = "<?php   if (!empty($staff_id)) { echo $getStaff[0]['username']; } ?>"  placeholder="Username" ></div>
												    <p style="color: red;" id="comment"></p> 
												</div>
												<div class="form-group"><label class="col-sm-3 control-label">Password <span class="symbol required"></span></label>
														<div class="col-sm-7"><input type="password" class="form-control" id="password" name="password"  value = "<?php  if (!empty($staff_id)) {  echo $getStaff[0]['password']; } ?>"  placeholder="Password"></div>
												</div>
												<div class="form-group"><label class="col-sm-3 control-label">Confirm Password <span class="symbol required"></span></label>
														<div class="col-sm-7"><input type="password" class="form-control" id="password_again" name="password_again"  value = "<?php   if (!empty($staff_id)) { echo $getStaff[0]['password']; } ?>"   placeholder="Confirm Password"></div>
												</div>
										       <div class="form-group"><div class="col-sm-2 col-sm-offset-3"><button class="btn btn-light-grey back-step btn-block"><i class="fa fa-circle-arrow-left"></i> Back</button></div>
												<div class="col-sm-2 col-sm-offset-3"><button class="btn btn-blue next-step btn-block">Next <i class="fa fa-arrow-circle-right"></i></button>
												</div>
